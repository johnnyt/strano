...
!
    ...
    end_time = Time.now
    STDOUT.puts "\n\n-----[ #{end_time.to_formatted_s(:db)} | SNAPSHOT COMPLETE | Took #{((end_time - start_time).to_f / 60.0).round(2)} minutes ]\n"
  end
  ...
  task :restore => :environment do
    ...
    if File.exist?('snapshot_dirs.tar')
      ...
      end_time = Time.now
      STDOUT.puts "\n\n-----[ #{end_time.to_formatted_s(:db)} | RESTORE COMPLETE | Took #{((end_time - start_time).to_f / 60.0).round(2)} minutes ]\n\n"
    end
